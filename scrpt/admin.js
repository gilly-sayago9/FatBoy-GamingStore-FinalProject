const admin = {
  ...
  renderOverview: (users, games) => {
    ...
    document.getElementById("stat-users").innerText = users.filter(
      (u) => u.role !== "admin",
    ).length;
  ...
  renderGamesTable: (games) => {
    ...
    if (games.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" style="text-align:center;">No games in inventory.</td></tr>';
      return;
    }
    tbody.innerHTML = games
      .map(
        (g) =>
          `<tr><td><img src="${g.img}" class="table-img" onerror="this.src='images/logo.png'"></td><td style="font-weight:600; color:white;">${g.title}</td><td>$${g.price}</td><td><button class="action-btn btn-edit" onclick="admin.editGame(${g.id})"><i class="fas fa-pen"></i></button><button class="action-btn btn-delete" onclick="admin.deleteGame(${g.id})"><i class="fas fa-trash"></i></button></td></tr>`,
      )
      .join("");
  },
  ...
  handleGameForm: async (e) => {
    ...
    try {
      ...
      if (fileInput.files[0]) {
        imgUrl = await database.uploadImage(fileInput.files[0]);
      }
      let hoverImgUrl = "";
      if (hoverInput.files[0]) {
        hoverImgUrl = await database.uploadImage(hoverInput.files[0]);
      }
      ...
      Swal.fire({
        icon: "success",
        title: "Success",
        text: idInput ? "Game Updated!" : "Game Added!",
      });
  ...
  deleteGame: async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: "Game removed from inventory.",
      });
    }
  },
